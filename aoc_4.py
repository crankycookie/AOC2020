...
def parseInput(input):
  ...
  for passport in passports:
    ...
    for field in fields:
      colonPos = field.find(":")
      key = field[:colonPos]
      val = field[colonPos+1:]
      parsedPassport[key] = val
    parsedPassports.append(parsedPassport)
...
def isPassportValid(passport):
  try:
    ...
    byr = passport["byr"]
    if len(byr) != 4:
      raise Exception("byr not long enough")
    
    if int(byr) < 1920 or int(byr) > 2002:
      raise Exception("byr date wrong")

    #iyr (Issue Year)
    iyr = passport["iyr"]
    if len(iyr) != 4:
      raise Exception("iyr not long enough")
    
    if int(iyr) < 2010 or int(iyr) > 2020:
      raise Exception("iyr date wrong")

    #eyr (Expiration Year)
    eyr = passport["eyr"]
    if len(eyr) != 4:
      raise Exception("eyr not long enough")
    
    if int(eyr) < 2020 or int(eyr) > 2030:
      raise Exception("eyr date wrong")

    #hgt (Height)
    hgt = passport["hgt"]
    hasUnit = False
    if "cm" in hgt and "in" in hgt:
      raise Exception("hgt too many units")
    if "in" in hgt:
      hasUnit = True
      unitPos = hgt.find("in")
      val = int(hgt[:unitPos])
      if val < 59 or val > 76:
        raise Exception("hgt size invalid (in)")
    if "cm" in hgt:
      hasUnit = True
      unitPos = hgt.find("cm")
      val = int(hgt[:unitPos])
      if val < 150 or val > 193:
        raise Exception("hgt size invalid (cm)")
    if not hasUnit:
      raise Exception("hgt missing unit")
    

    #hcl (Hair Color)
    hcl = passport["hcl"]
    if len(hcl) != 7:
      raise Exception("hcl bad length")
    if hcl[0] != '#':
      raise Exception("hcl missing #")
    int(hcl[-6:], 16)

    #ecl (Eye Color)
    ecl = passport["ecl"]
    if ecl not in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
      raise Exception("ecl bad value")

    #pid (Passport ID)
    pid = passport["pid"]
    if len(pid) != 9:
      raise Exception("pid bad length")
    if int(pid) < 0:
      raise Exception("pid bad value")

    #cid (Country ID) - optional
    #valid = valid and "cid" in keys
...
validPassports = 0
for passport in parseInput(input):
