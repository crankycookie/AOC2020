...
def parseLine(line):
  ...
  parent = splitLine[0].rstrip(" ")
  children = splitLine[1].rstrip(".").lstrip(" ").split(",")
  ...
  for child in children:
    # ick
    parsedChild = child.strip().replace(" bags", "").replace(" bag", "").split(" ")
    childCount = parsedChild[0]
    childColor = parsedChild[1] + " " + parsedChild[2]
    childrenMap[childColor] = childCount
  return (parent, childrenMap)

def createRulesFromList(parsedList):
  rules = []
  for item in parsedList:
    rules.append(parseLine(item))
  return rules

def getAllowedBagsFromRules(rules, bagColor):
  allowedBags = set()
  for rule in rules:
    childrenMap = rule[1]
    parent = rule[0]
    try:
      childrenMap[bagColor]
  ...
  for allowedBag in allowedBags:
    childBags = getAllowedBagsFromRules(rules, allowedBag)
    allowedBags = allowedBags.union(childBags)

  return allowedBags

rules = createRulesFromList(listInput)

# Part 1
allowedBags = getAllowedBagsFromRules(rules, "shiny gold")
print("Allowed Bags:", allowedBags)

# Part 2
def lookupChildBagsAndCount(rules, bagColor):
  for rule in rules:
    if rule[0] == bagColor:
      return rule[1]

def getChildBagCount(rules, bagColor):
  result = 0
  for rule in rules:
    parentBag = rule[0]
    if parentBag == bagColor:
      childrenMapping = rule[1]
      if childrenMapping == None:
        return result
      for childColor, childCount in childrenMapping.items():
        recursiveCount = getChildBagCount(rules, childColor)
        result = result + int(childCount) + (int(childCount) * recursiveCount)
        
  return result

print("Total Bags:", getChildBagCount(rules, "shiny gold"))
